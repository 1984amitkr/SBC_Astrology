...
def _calc_ut_compat(jd_ut, body, flags):
    """Compatibility wrapper for Swiss Ephemeris Python APIs.

    Depending on the installed pyswisseph/pysweph build, calc_ut() may
    return either (xx, retflags) or (xx, retflags, serr).  The application
    only needs the position tuple and the return flags, so normalize both
    forms here.
    """
    result = swe.calc_ut(jd_ut, body, flags)
    if not isinstance(result, tuple):
        raise RuntimeError(f'Unexpected Swiss Ephemeris calc_ut result: {type(result)!r}')
    if len(result) == 2:
        xx, retflags = result
    elif len(result) == 3:
        xx, retflags, _serr = result
    else:
        raise RuntimeError(f'Unexpected Swiss Ephemeris calc_ut return length: {len(result)}')
    return xx, retflags
...
def sidereal_longitude(jd_ut, body):
    ...
    if body == 'Ketu':
        xx, _ = _calc_ut_compat(jd_ut, swe.MEAN_NODE, flags)
        # Ketu is 180 degrees from Rahu and has the opposite longitudinal speed.
        return norm360(xx[0] + 180), -xx[3]
    xx, _ = _calc_ut_compat(jd_ut, PLANETS[body], flags)
